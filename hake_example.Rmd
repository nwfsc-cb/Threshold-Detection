...
pdf("figurepdfs/hake_fits_520.pdf", height = 3.5)
par(mfrow = c(1, 2))
...
abline(v = thresh_5_20_summ$thresh_mean[which(thresh_5_20_summ$thresh_method=="abs_max_d2"& thresh_5_20_summ$sig_type=="none")], col = "blue", lty = 2)
...
# plot results
pdf("figurepdfs/hake_fits_all.pdf")
par(mfrow = c(2, 2))
par(mar=c(0.5, 1, 1, 0), oma = c(3, 2.5, 1, 0.5))
# total
...
points(x = dt_sub$FPR[which(dt_sub$obs_error==obs_set_h[i] & dt_sub$ts_length==ts_set_h[j])], y = dt_sub$TPR[which(dt_sub$obs_error==obs_set_h[i] & dt_sub$ts_length==ts_set_h[j])], pch = pt_pch[j], col = pt_cols[i]) #adjustcolor(pt_cols[i], alpha.f = pt_alpha[j])
}  
}
