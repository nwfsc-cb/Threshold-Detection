...
#' smooth_type: type of smooth used with gams, defaults to "tp"

gamfun <- function(nsim.f, simdt.f, knots.f, xvals.f, smooth_type = "tp"){
  ...
  for(i in 1:nsim.f){
    ...
    gam_i <- gam(obs_response ~ s(driver,k=knots.f,bs=smooth_type),data = datIN) # s() defines smoother in the GAM: takes list of covariate variables that the smoother will be a function of (here, "driver"); k = number of knots (wiggliness of smoother); bs indicates the (penalized) smoothing basic to use ("tp" means thin plate regression spline)
