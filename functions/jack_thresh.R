#' function for evaluating thresholds in simulated data using jackknifing and functions for
#' calculating threshold locations for different threshold definitions
...
jack_thresh <- function(simdt, x_pred, xlength = 50, z_scale = TRUE, thresh_methods = c("abs_max_d2", "min_d2", "zero_d2", "zero_d1"),
  ...
  # length of sig_criteria vector (for determining number of rows to have in holding matrices)
  ...
  # make the holding matrices to results: number of rows = number of simulations x number of significance
  # criteria used, number of columns = number of threshold definitions
  ...
  for(i in 1:nsim){ # for each simulation
    ...
    # get the xvals (driver values to use for generating the predicted derivative values)
    if(is.na(x_pred[1])==T){
    ...
    # first fit a gam to the whole data set and see whether a threshold was detected for each threshold definition
   ...
   # }
    ...
    # check with threshold definitions (if any) found a threshold on the full data set for each sig criteria
    methods_check <- matrix(NA, nrow = length(sig_criteria), ncol = length(thresh_methods))
    ...
    for(mm in 1:length(thresh_methods)){ # for each threshold definition
      ...
      if(thresh_method=="abs_max_d2"){
        ...
        for(ss in 1:length(sig_criteria)){ # for each sig criteria

          sig_criteria_s <- sig_criteria[ss]

          if(sig_criteria_s=="none"){
            # use abs_max_threshF function (see below) to check whether a peak in the second deriv was detected in the full data set
          ...
          if(sig_criteria_s == "jack_quant"){
            methods_check[ss, mm] <- NA # can't evaluate full data using jackknifed quantiles as sig criteria w/o having done the jackknifing
          }
          ...
          # get the index (row) for storing these results in the holding matrix
          add_index <- ss-1
      ...
      if(thresh_method=="min_d2"){
        ...
        for(ss in 1:length(sig_criteria)){
          ...
          if(sig_criteria_s=="none"){
            # use min_threshF function (see below) to check whether a min in the second deriv was detected in the full data set
      ...
      if(thresh_method=="zero_d2"){
        ...
        for(ss in 1:length(sig_criteria)){
          ...
          if(sig_criteria_s=="none"){
            # use root_threshF function (see below) to check whether a root in the second deriv was detected in the full data set
      ...
      if(thresh_method=="zero_d1"){
        ...
        for(ss in 1:length(sig_criteria)){
          ...
          if(sig_criteria_s=="none"){
            # use min_threshF function (see below) to check whether a min in the first deriv was detected in the full data set
    ...
    } # end of threshold definitions check on full data set
   ...
   # } else{
    ...
    # now do the leave-one-out jackknifing resampling

    dd <- datIN # make data set for the jackknife resampling
    dd <- dd[ ,c("driver", "obs_response")]

    # make holding matrices for the derivatives for each jackknife iteration
    ...
    for(int in 1:length(dd[,1])){ # for each observation
      ...
      # get the first and second derivatives and their simultaneous intervals for this jackknife iteration
      ...
      # get the 2.5% and 97.5% quantiles of the jackknifed first and second derivatives (for applying jack quant significance criteria)
      ...
      # make holding lists for results for the different threshold definitions
      thresh_l <- vector(mode = "list", length = length(thresh_methods))
      # put place holder matrixes in the list
      for(ll in 1:length(thresh_l)){
        thresh_l[[ll]] <- matrix(NA, nrow = length(dd[,1]), ncol = length(sig_criteria)) # will fill with threshold values for each jackknife iteration and sig criteria for the ll^th threshold definition
      }
      ...
      if(thresh_method =="abs_max_d2"){
          ...
          for(ss in 1:length(sig_criteria)){ # for each significance criteria

            sig_criteria_s <- sig_criteria[ss]

            # calculate the max(|s''(x)|) threshold in the jackknife iteration
      ...
      if(thresh_method =="min_d2"){
        ...
        for(ss in 1:length(sig_criteria)){
          ...
          # calculate the min(s''(x)) threshold in the jackknife iterations
      ...
      if(thresh_method =="zero_d2"){
        ...
        for(ss in 1:length(sig_criteria)){
          ...
          # calculate the s''(x) == 0 threshold in the jackknife iterations
     ...
     } # end of threshold calculations for each threshold definition

  } # end of iterations across jackknife iterations


    # final threshold calculations: summarize across jackknife iterations
    for(mm in 1:length(thresh_methods)){ # for each threshold definition
    ...
    # get the mean and SEs (across jackknife iterations) of the chosen thresholds for the ith simulation
 ...
 # } # end of if else for whether methods_check found a threshold
  ...
  thresh_methods_ss <- rep(thresh_methods, each = length(sig_criteria)) # vector with threshold definitions
  nsim_ss <- rep(c(1:nsim), each = length(sig_criteria)) # vector with simulation numbers

  df <- data.frame(
    sim = rep(nsim_ss, length(thresh_methods)), # simulation number
    thresh_method = rep(thresh_methods_ss, each = nsim), # threshold definition
    thresh_mean = c(thresh_mean),# mean threshold estimate; need c() to turn from matrix into a vector
    ...
    thresh_se = c(thresh_se), # SE of threshold estimates across jackknife iterations
    sig_type = rep(sig_criteria, nsim*length(thresh_methods)), # significance criteria used
    #thresh_detected = c(thresh_detected),
    ci_n_up = c(ci_n_up), # upper CI of threshold estimates (normal approx)
    ci_n_low = c(ci_n_low), # lower CI of threshold estimates (normal approx)
    ci_em_up = c(ci_em_up), # upper CI of threshold estimates (empirical -> 97.5% quantile)
    ci_em_low = c(ci_em_low), # lower CI of threshold estimates (empirical -> 2.5% quantile)
    thresh_diff = c(c(thresh_mean)-thresh_loc) # difference between mean threshold estimate and true threshold location
...
# functions for finding thresholds in the derivatives and determining their significance
# common inputs:
...
# function for calculating thresholds defined as the location where the absolute value of the second deriv is greatest
abs_max_threshF <- function(deriv_i, sig_type, xvals, deriv_up, deriv_low, pkht, pkup){

 # first get the max and min points in the derivative
  # mins
  mins1 <- findpeaks(-deriv_i, nups =pkup) # findpeaks returns NULL if none are found

  if(is.null(mins1)==F){# if mins were found

  min_heights <- mins1[,1] # first column of findpeaks output is abs value of peak height

  mins2 <- matrix(mins1[which(min_heights>=pkht),], ncol = 4) # subset the findpeaks output for mins  with abs values greater than or equal to pkht
  ...
  # repeat for max's
  maxs1 <- findpeaks(deriv_i, nups =pkup)

  if(is.null(maxs1)==F){ # if maxes were found

    max_heights <- maxs1[,1]

    maxs2 <- matrix(maxs1[which(max_heights>=pkht),], ncol = 4)
  ...
  if(is.null(min_all)==T & is.null(max_all)==T){ # if no peaks were found
    abs_max_thresh <- NA#which(1==10)#length(min_all) # abs_max_thresh = integer(0)

  } else{ # if peaks were found, determine whether they are significant and which one is greatest
...
# ##### #### min in second deriv
# function for calculating thresholds defined as the location with the smallest (largest magnitude) minima in the second derivative
# same as the above function except only looking for mins in the second derivative, not mins and maxes
min_threshF <- function(deriv_i, sig_type, xvals, deriv_up, deriv_low, pkht, pkup){

  # first get the min points
  mins1 <- findpeaks(-deriv_i, nups =pkup)

  if(is.null(mins1)==F){# if at least one min was found

    min_heights <- mins1[,1]

    mins2 <- matrix(mins1[which(min_heights>=pkht),], ncol = 4)
...
# ##### #### roots
# function for calculating thresholds defined as roots in the first or second derivative (location where the derivative crosses zero)
root_threshF <- function(deriv_i, sig_type, xvals, deriv_up, deriv_low){

  # find the points where the deriv crosses zero (=changes sign)
  # sign(-10) = -1, sign(10) = 1, sign(0) = 0
  droots <- which(diff(sign(deriv_i))!=0)

  if(length(droots)==0){ # if there were no roots
    ...
    if(sig_type=="none"){
      ...
      } else{ # determine which is significant/most likely to correspond to the threshold
...
# first get which roots are significant (those that are closest to where the upper/lower intervals cross zero)
      ...
      if(length(d_low_roots) > 0 & length(d_up_roots) > 0){ # if both intervals had roots
        ...
        # if there weren't any, there were no significant roots
        if(length(na_chk)==0){
